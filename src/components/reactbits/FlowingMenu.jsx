...
const repeatedMarqueeContent = Array.from({ length: 4 }).map((_, idx) => (
  <div key={idx} className="flex items-center gap-[2vw] px-[2vw]">
    <div
      className="w-[200px] h-[7vh] rounded-[50px] bg-cover bg-center flex-shrink-0"
      style={{ backgroundImage: `url(${image})` }}
    />
    <p className="w-max text-[#060010] uppercase font-normal text-[4vh] leading-[1.2]">
      {text}
    </p>
  </div>
));
  ...
  return (
    <div className="flex-1 relative overflow-hidden text-center shadow-[0_-1px_0_0_#fff]" ref={itemRef}>
      ...
      >
        ...
        >
          <div className="flex">{repeatedMarqueeContent}</div>
          <div className="flex">{repeatedMarqueeContent}</div>
        </div>
